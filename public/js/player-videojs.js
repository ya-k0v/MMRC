...
function setPlayerBadge(version) {
  if (!playerBadge) return;
  const resolvedVersion = String(version || '').trim() || APP_VERSION;
  playerBadge.textContent = `ID: ${badgeDeviceId} | v${resolvedVersion}`;
}

function extractClientVersionFromDevice(device) {
  if (!device || typeof device !== 'object') return null;

  if (typeof device.appVersion === 'string' && device.appVersion.trim()) {
  ...
  const explicitMatch = platform.match(/(?:MMRC|VC)\s*([0-9]+\.[0-9]+\.[0-9]+)/i);
  if (explicitMatch && explicitMatch[1]) {
    return explicitMatch[1];
  }

  const genericMatch = platform.match(/([0-9]+\.[0-9]+\.[0-9]+)/);
  return genericMatch && genericMatch[1] ? genericMatch[1] : null;
}

async function resolvePreviewClientVersion() {
  if (!preview || !device_id) return;

  try {
    const accessToken = localStorage.getItem('accessToken');
    const headers = accessToken ? { Authorization: `Bearer ${accessToken}` } : undefined;
    const response = await fetch('/api/devices', { cache: 'no-store', headers });
    if (!response.ok) return;

    const devices = await response.json();
    if (!Array.isArray(devices)) return;

    const device = devices.find((item) => item && item.device_id === device_id);
    const clientVersion = extractClientVersionFromDevice(device);
    if (clientVersion) {
      setPlayerBadge(clientVersion);
    }
  } catch (error) {
    console.warn('[Player] Failed to resolve preview client version:', error?.message || error);
  }
}

setPlayerBadge(previewClientVersionParam || APP_VERSION);
if (preview && device_id && !previewClientVersionParam) {
  void resolvePreviewClientVersion();
}
...
if (!device_id || !device_id.trim()) {
          ...
          if (preview && previewFile) {
            ...
            setTimeout(() => {
              ...
              if ((previewType === 'pdf' || resolvedPreviewType === 'pdf') && previewPage) {
                ...
                if (previewStreamUrl && (!device_id || !previewFile)) {
  ...
  function isLikelyCorsOrAccessFetchError(error, url) {
    const name = String(error?.name || '').toLowerCase();
    const message = String(error?.message || '').toLowerCase();
    const failedToFetch = name === 'typeerror' || message.includes('failed to fetch') || message.includes('networkerror');
    return failedToFetch && isCrossOriginUrl(url);
  }
