import { ensureAuth, adminFetch, logout } from './admin/auth.js';
...
function renderHeroList() {
  ...
  if (!state.filtered.length) {
    state.listEl.innerHTML = '<li class="hero-list__item hero-list__empty">Ничего не найдено</li>';
    return;
  }
  ...
  state.listEl.innerHTML = itemsToShow
    .map((hero) => {
...
function bindSearch() {
  ...
  state.searchEl.addEventListener('input', (e) => {
    ...
    state.filtered = term
      ? state.heroes.filter((hero) => (hero.full_name || '').toLowerCase().includes(term))
      : [...state.heroes];
...
function renderHeroDetail(hero) {
  ...
  state.detailEl.style.display = 'block';
        ...
        <div class="hero-card-right" style="position: relative;">
          ...
          <div class="hero-scroll">
            <div class="hero-scroll__bio" data-edit="biography" data-type="multiline" data-placeholder="Добавьте биографию">
              ${hero.biography ? escapeHtml(hero.biography) : '<em style="color:var(--muted);">Информация отсутствует</em>'}
            </div>
          </div>
        </div>
      </div>
...
//  hint.textContent = type === 'multiline' ? 'Enter — сохранить, Esc — отмена' : 'Enter — сохранить, Esc — отмена';
  ...
  const onKeydown = (event) => {
    if (event.key === 'Enter' && type !== 'multiline' && !event.shiftKey) {
...
function renderMediaThumbnail(media, index) {
  return `
    <div class="media-thumbnail" data-index="${index}" style="position: relative;">
      ${
        media.type === 'photo'
      ...
      <button class="hero-media-thumbnail-delete" data-action="remove-media" data-id="${media.id}" data-index="${index}" title="Удалить материал">×</button>
    </div>
  `;
}
