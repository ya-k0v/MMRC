...
function openServiceLogsModal(adminFetch) {
  ...
  setTimeout(() => {
    const levelSelect = document.getElementById('serviceLogsLevelSelect');
    const moduleSelect = document.getElementById('serviceLogsModuleSelect');
    ...
    autoscrollEl.onchange = () => {
      ...
      if (serviceLogsViewerState.autoScroll) outputEl.scrollTop = outputEl.scrollHeight;
    };
    ...
    return `
      <div class="item modal-user-row ${isSelected ? 'selected' : ''}" style="display:flex; justify-content:space-between; align-items:center; gap:var(--space-sm);" onclick="selectUserInUsersModal(${safeUserId})">
        ...
        <div style="display:flex; gap:4px; flex-shrink:0;">
          ${window.currentUser && window.currentUser.role === 'admin' ? `<button class="secondary" style="min-width:auto; padding:6px 10px; display:flex; align-items:center; justify-content:center;" onclick="event.stopPropagation(); editUserInModal(${safeUserId}, ${usernameArg}, ${escapeJsStringForAttr(u.full_name || '')}, ${roleArg})" title="Редактировать">${getSearchIcon(16)}</button>` : ''}
