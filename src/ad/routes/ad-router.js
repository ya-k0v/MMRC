...
export function createAdRouter(deps = {}) {
  ...
  router.get('/analytics/stats', adminOrAuth, async (req, res) => {
    try {
      ...
      const toStr = (v) => v instanceof Date ? v.toISOString() : (v ? String(v) : null);
      ...
      const summary = {
        ...
        first_date: rows.length ? toStr(rows[rows.length - 1].played_at) : null,
        last_date: rows.length ? toStr(rows[0].played_at) : null,
      ...
      const perFileMap = {};
      for (const r of rows) {
        const ps = toStr(r.played_at);
        const d = ps ? ps.substring(0, 10) : 'unknown';
        const h = ps ? ps.substring(11, 13) : '00';
